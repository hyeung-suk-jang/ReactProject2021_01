...
const Books = ({ detail }) => {
  ...
  return (
    <div className="result">
      <span className="title txt_black">
        <strong>{detail.title_info._cdata}</strong>
      </span>
      ...
      <span className="pub_year_info detail_info">
        {detail.pub_year_info._text} |{" "}
      </span>
      <span className="reg_date detail_info">
        비치일: {detail.reg_date._text} |{" "}
      </span>
      <span className="call_no detail_info">
        청구기호: {detail.call_no._cdata}
      </span>
      <div className="row_space" style={{ height: "9px" }}></div>
      <span className="txt_black useinfo">자료 이용장소:</span>
      <span className="txt_orange place_info"> {detail.place_info._text}</span>
      <div className="double_space"></div>
      {borrowBooks.find((v) => v.id === detail.id._text) ? (
        <button className="return_btn" onClick={onClickReturn}>
          반납하기
        </button>
      ) : (
        <button className="borrow_btn" onClick={onClickBorrow}>
          대출하기
        </button>
      )}
    </div>
